...
class Hour {
  ...
  static create(userId, date, startTime, endTime, callback) {
    const sql =
      "INSERT INTO hours (userId, date, startTime, endTime) VALUES (?, ?, ?, ?)";
  ...
  static findByUserId(userId, callback) {
    ...
    db.all(sql, [userId], (err, rows) => {
      ...
      const hours = rows.map(
        (row) =>
          new Hour(row.id, row.userId, row.date, row.startTime, row.endTime)
      );
      callback(null, hours);
    });
  }
  ...
  static update(id, userId, date, startTime, endTime, callback) {
    const sql =
      "UPDATE hours SET userId = ?, date = ?, startTime = ?, endTime = ? WHERE id = ?";
